...
def _parse(body):
    ...
    try:
        ...
        _logger.error(f'{deepl_error_message} Caught IndexError, unexpected JSON: {body}')


def _fetch(text, url, key):
    """Makes HTTP POST request to DeepL translation API, returns JSON string"""
    data = parse.urlencode({'auth_key': key, 'text': text, 'target_lang': 'EN'}).encode()
    ...
    try:
        ...
        if resp.status != 200:
            _logger.error(f'{deepl_error_message} Response: HTTP {resp.status}, {resp.reason}')
        return resp.read()
